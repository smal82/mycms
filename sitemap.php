...
try {
    $stmt = $pdo->prepare("SELECT slug, updated_at FROM " . DB_PREFIX . "posts WHERE status='pubblicato' AND post_type = 'post' AND deleted_at IS NULL ORDER BY created_at DESC");
    $stmt->execute();
    while ($post = $stmt->fetch()) {
        echo "    <url>\n";
        echo "        <loc>" . htmlspecialchars($baseUrl . '/post/' . $post['slug'], ENT_XML1, 'UTF-8') . "</loc>\n";
...
// Portfolio (se esiste)
try {
    $stmt = $pdo->prepare("SELECT slug, updated_at FROM " . DB_PREFIX . "posts WHERE status='pubblicato' AND post_type = 'portfolio' AND deleted_at IS NULL ORDER BY created_at DESC");
    $stmt->execute();
