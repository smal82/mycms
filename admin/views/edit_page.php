<h1><?php echo $page ? 'Modifica Pagina' : 'Nuova Pagina'; ?></h1>

<form method="POST" action="index.php?action=save_page" enctype="multipart/form-data">
    <?php if ($page): ?>
    ...
    <div class="form-group">
        <label>Titolo:</label>
        <input type="text" name="title" id="page-title" value="<?php echo $page ? htmlspecialchars($page['title']) : ''; ?>" required>
    </div>
    
    <div class="form-group">
        <label>Slug:</label>
        <input type="text" name="slug" id="page-slug" value="<?php echo $page ? htmlspecialchars($page['slug']) : ''; ?>" required>
    ...
    <div id="featured-image-preview" style="margin-top:10px;">
        <?php if ($page && $page['featured_image']): ?>
            <div style="position:relative; display:inline-block;">
                <img src="/uploads/<?php echo htmlspecialchars($page['featured_image']); ?>" style="max-width:200px;">
            ...
            <input type="hidden" name="existing_featured_image" value="<?php echo htmlspecialchars($page['featured_image']); ?>">
        <?php endif; ?>
    </div>
</div>
    ...
    <div class="form-group">
        ...
        <textarea name="content" id="page-content"><?php echo $page ? $page['content'] : ''; ?></textarea>
    </div>
    
    <div class="form-group">
        <label>Stato:</label>
        <select name="status" required>
            <option value="bozza" <?php echo ($page && $page['status'] === 'bozza') ? 'selected' : ''; ?>>Bozza</option>
            <option value="pubblicato" <?php echo ($page && $page['status'] === 'pubblicato') ? 'selected' : ''; ?>>Pubblicato</option>
        </select>
    </div>
    
    <button type="submit" class="btn">Salva Pagina</button>
    <a href="index.php?action=pages" class="btn btn-secondary">Annulla</a>
</form>
...
$('#page-slug').on('input', function() {
    $(this).data('manual', true);
});

// Jodit Editor
const editor = Jodit.make('#page-content', {  // o #post-content per edit_post.php
    height: 500,
    language: 'it',
    ...
    // Usa i plugin integrati di Jodit
    useImageEditor: true,
