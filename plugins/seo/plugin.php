...
class SeoPlugin {
    ...
    public function renderSitemapViewer() {
        ...
        try {
            $stmt = $db->pdo->query("SELECT COUNT(*) as count FROM " . DB_PREFIX . "posts WHERE status='pubblicato' AND deleted_at IS NULL");
        ...
        try {
            $stmt = $db->pdo->query("SELECT COUNT(*) as count FROM " . DB_PREFIX . "portfolio WHERE status='pubblicato' AND deleted_at IS NULL");
        ...
        if ($postsCount > 0) {
            ...
            try {
                $stmt = $db->pdo->query("SELECT slug, updated_at FROM " . DB_PREFIX . "posts WHERE status='pubblicato' AND deleted_at IS NULL ORDER BY updated_at DESC LIMIT 50");
                while ($post = $stmt->fetch()) {
                    echo '<tr>';
                    echo '<td style="padding: 10px; border-bottom: 1px solid #dee2e6;"><a href="' . $siteUrl . '/blog/' . htmlspecialchars($post['slug']) . '" target="_blank" style="color: #007bff;">blog/' . htmlspecialchars($post['slug']) . '</a></td>';
